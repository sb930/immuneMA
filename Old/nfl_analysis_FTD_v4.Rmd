...
---
title: "nfl_analysis_v4.Rmd"
author: "Sanjay"
date: "24/12/2022"
output: html_document
---

# Step 1: run a loop for nfl for controls to predict mean and sd where mdian, IQR, range available
...
nfl_df3_completes_con <- subset(nfl_df2_con, complete.cases(nfl_df2_con[, c("l_range_nfl_con", "l_iqr_nfl_con", "med_nfl_con", "u_iqr_nfl_con", "u_range_nfl_con")]))

# Loop through all rows of the data frame "data2"

for (i in 1:nrow(nfl_df3_completes_con)) {
  if (nrow(nfl_df3_completes_con) == 0) {
...
for (i in 1:nrow(nfl_df4_incompletes_con)) {
  if (nrow(nfl_df4_incompletes_con) == 0) {
      # If the subset command returned no values, skip the rest of the code chunk
      next
    } else {
...
for (i in 1:nrow(nfl_df4_all_con)) {
  if (nrow(nfl_df4_all_con) == 0) {
      # If the subset command returned no values, skip the rest of the code chunk
      next
    } else {
...
nfl_df3_completes_ftd <- subset(nfl_df2_ftd, complete.cases(nfl_df2_ftd[, c("l_range_nfl_ftd", "l_iqr_nfl_ftd", "med_nfl_ftd", "u_iqr_nfl_ftd", "u_range_nfl_ftd")]))

# Loop through all rows of the data frame "data2"

for (i in 1:nrow(nfl_df3_completes_ftd)) {
  if (nrow(nfl_df3_completes_ftd) == 0) {
    # If the subset command returned no values, skip the rest of the code chunk
    next
  } else {
    #for (i in 1:nrow(nfl_df3_completes_ftd)) {
...
for (i in 1:nrow(nfl_df4_incompletes_ftd)) {
  if (nrow(nfl_df4_incompletes_ftd) == 0) {
      # If the subset command returned no values, skip the rest of the code chunk
      next
    } else {
      nfl_df4_incompletes_ftd$sd <- format(as.numeric(nfl_df4_incompletes_ftd$sd), scientific = FALSE)
  }
}
...
# Order the data frame by the "study.id" column
for (i in 1:nrow(nfl_df4_all_ftd)) {
  if (nrow(nfl_df4_all_ftd) == 0) {
      # If the subset command returned no values, skip the rest of the code chunk
      next
    } else {
...
# Add a "value" column to the nfl_df4_all_con data frame with estimated
if (nrow(nfl_df4_all_ftd) > 0) {
  nfl_df4_all_ftd$value <- "extracted"
}

# Add a "value" column to the data_t_nfl_con data frame with extracted
if (nrow(data_t_nfl_ftd) > 0) {
