...
MIP1a_df4_completes_con <- (matrix(nrow = 0, ncol = length(columns)))
colnames(MIP1a_df4_completes_con) <- columns
...
for (i in 1:nrow(MIP1a_df3_completes_con)) {
  if (nrow(MIP1a_df3_completes_con) == 0) {
    ...
    #for (i in 1:nrow(MIP1a_df3_completes_con)) {
      ...
      # MIP1a_df4_completes_con <- cbind(data.frame(mean = meansd$est.mean, sd = meansd$est.sd, study.id = row$studyid))
        # Store the results in the results data frame
      MIP1a_df4_completes_con <- rbind(MIP1a_df4_completes_con, data.frame(mean = meansd$est.mean, sd = meansd$est.sd, study.id = row$studyid, n_con = row$n_con))
    }
  }
#}

print(MIP1a_df4_completes_con)
...
if (nrow(MIP1a_df4_all_con) > 0) {
  MIP1a_df4_all_con$value <- "extracted"
}
...
MIP1a_df4_completes_ftd <- (matrix(nrow = 0, ncol = length(columns)))
colnames(MIP1a_df4_completes_ftd) <- columns

MIP1a_df3_completes_ftd <- subset(MIP1a_df2_ftd, complete.cases(MIP1a_df2_ftd[, c("l_range_MIP1a_ftd", "l_iqr_MIP1a_ftd", "med_MIP1a_ftd", "u_iqr_MIP1a_ftd", "u_range_MIP1a_ftd")]))

# Loop through all rows of the data frame "data2"

for (i in 1:nrow(MIP1a_df3_completes_ftd)) {
  if (nrow(MIP1a_df3_completes_ftd) == 0) {
    # If the subset command returned no values, skip the rest of the code chunk
    next
  } else {
    #for (i in 1:nrow(MIP1a_df3_completes_ftd)) {
      ...
      # # Store the results in the results data frame
      # MIP1a_df4_completes_ftd <- cbind(data.frame(mean = meansd$est.mean, sd = meansd$est.sd, study.id = row$studyid))
        # Store the results in the results data frame
      MIP1a_df4_completes_ftd <- rbind(MIP1a_df4_completes_ftd, data.frame(mean = meansd$est.mean, sd = meansd$est.sd, study.id = row$studyid, n_ftd = row$n_ftd))
    }
  }
#}

print(MIP1a_df4_completes_ftd)
...
# Add a "value" column to the MIP1a_df4_all_con data frame with estimated
if (nrow(MIP1a_df4_all_ftd) > 0) {
  MIP1a_df4_all_ftd$value <- "extracted"
}
